...
namespace OgmoPipelineExtensions
{
    ...
    internal static class ColorHelper
    {
        ...
        internal static Color FromRGBAAttributes(XmlNode node, string name)
        {
            var colorNode = node.SelectSingleNode(name);

            if (colorNode != null)
            {
                int a = int.Parse(colorNode.Attributes["A"].Value, CultureInfo.InvariantCulture);
                int r = int.Parse(colorNode.Attributes["R"].Value, CultureInfo.InvariantCulture);
                int g = int.Parse(colorNode.Attributes["G"].Value, CultureInfo.InvariantCulture);
                int b = int.Parse(colorNode.Attributes["B"].Value, CultureInfo.InvariantCulture);
